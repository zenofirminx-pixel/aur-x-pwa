...
function initApp() {
  // Init theme au chargement
  if (localStorage.getItem('codeTheme') === 'light') {
  ...
  function formatMessage(text) {
    ...
    if (!text.startsWith("<h") && !text.startsWith("<ul") && !text.startsWith("<pre") && !text.startsWith("<blockquote") && !text.startsWith('<div class="code-header"')) {
      text = "<p>" + text + "</p>";
    }
    
    // BLOC CODE AVEC COLLAPSE + NUMÉROS DE LIGNE + LABEL - BALISE FERMÉE
    codeBlocks.forEach((block, i) => {
      const escapedCode = block.code.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
      const lines = escapedCode.split('\n');
      const shouldCollapse = lines.length > 15;
      const linesHtml = lines.map(line => `<span class="line">${line || ' '}</span>`).join('');
      
      text = text.replace(`__CODE_${i}__`, `
        <div class="code-header">
          <span class="code-lang">${block.lang || 'text'}</span>
          <div class="code-actions">
            ${shouldCollapse ? `<button class="collapse-btn" onclick="toggleCollapse(this)">Développer</button>` : ''}
            <button class="copy-btn" onclick="copyCode(this)">Copier</button>
          </div>
        </div>
        <pre><code class="language-${block.lang}">${linesHtml}</code></pre>
      `);
    });
  ...
  async function sendMessage() {
    ...
    try {
      ...
      hideTypingIndicator();
      if (!res.ok) {
        const errorText = "⚠️ Le serveur AurX rencontre actuellement un problème.\nRéessaie dans quelques instants.";
      ...
      hideTypingIndicator();
      const errorText = "⚠️ Impossible de se connecter à AurX.\nVérifie ta connexion Internet puis réessaie.";
      const errTime = Date.now();
      addMessage(errorText, 'bot error', errTime);
      currentMessages.push({text: errorText, type: 'bot error', timestamp: errTime});
      saveConversation(msg.slice(0, 40), currentMessages);
